...
namespace FellowOakDicom.Tests
{
    ...
    public class DicomFileTest
    {
        ...
        public async Task SavingAndOpeningAVeryLargeDicomFile_ShouldWork()
        {
            ...
            {
                // Arrange
                var pixelDataLength = 4_294_967_298; // 4 GB + 2 bytes, so the file will be larger than uint.MaxValue
                var numberOfFrames = 2; // 2 GB + 1 byte, so each frame will be larger than int.MaxValue but smaller than uint.MaxValue
                ...
                originalDicomFile.FileMetaInfo.TransferSyntax = DicomTransferSyntax.ExplicitVRLittleEndian;
                originalDicomFile.Dataset.AddOrUpdate(DicomTag.SOPInstanceUID, DicomUIDGenerator.GenerateDerivedFromUUID());
                originalDicomFile.Dataset.AddOrUpdate(DicomTag.BitsAllocated, (ushort) 8);
                originalDicomFile.Dataset.AddOrUpdate(DicomTag.BitsStored, (ushort) 8);
                originalDicomFile.Dataset.AddOrUpdate(DicomTag.NumberOfFrames, numberOfFrames);
                originalDicomFile.Dataset.AddOrUpdate(pixelData);
