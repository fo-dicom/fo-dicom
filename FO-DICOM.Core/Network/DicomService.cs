    ...
    public abstract class DicomService : IDicomServiceRunner, IDisposable
    {
        ...
        private async Task SendNextPDUAsync()
        {
            while (IsConnected)
            {
                ...
                if (Options.LogDataPDUs && pdu is PDataTF pdatatf)
                {
                    Logger.InformationDataPduSent(LogID, pdatatf);
                }
                ...
                Options.ReportBytesSent?.Report(written);
            }
        }
            ...
            while (IsConnected)
            {
                try
                {
                    ...
                    do
                    {
                        ...
                        {
                            // disconnected
                            Logger.DebugNoDataFromConnectionStream();
                        ...
                        case RawPduType.A_ASSOCIATE_RQ:
                            {
                                ...
                                Logger.InformationAssociationRequestReceived(Association.CallingAE, Association);
                        ...
                        case RawPduType.A_ASSOCIATE_AC:
                            {
                                ...
                                Logger.InformationAssociationAcceptReceived(Association.CalledAE, Association);
                        ...
                        case RawPduType.A_ASSOCIATE_RJ:
                            {
                                ...
                                pdu.Read(raw);
                                Logger.InformationAssociationRejectedReceived(LogID, pdu.Result, pdu.Source, pdu.Reason);
                                if (this is IDicomClientConnection connection)
                                {
                        ...
                        case RawPduType.P_DATA_TF:
                            {
                                ...
                                {
                                    Logger.InformationDataPduReceived(LogID, pdu);
                                }
                        ...
                        case RawPduType.A_RELEASE_RQ:
                            {
                                ...
                                pdu.Read(raw);
                                Logger.InformationAssociationReleaseRequestReceived(LogID);
                                if (this is IDicomServiceProvider provider)
                                {
                        ...
                        case RawPduType.A_RELEASE_RP:
                            {
                                ...
                                pdu.Read(raw);
                                Logger.InformationAssociationReleaseResponseReceived(LogID);
                                if (this is IDicomClientConnection connection)
                                {
                        ...
                        case RawPduType.A_ABORT:
                            {
                                ...
                                pdu.Read(raw);
                                Logger.InformationAbortReceived(LogID, pdu.Source, pdu.Reason);
                ...
                {
                    // silently ignore
                    Logger.DebugConnectionDisposed(e);
                    await TryCloseConnectionAsync(force: true).ConfigureAwait(false);
                }
                catch (NullReferenceException e)
                {
                    // connection already closed; silently ignore
                    Logger.DebugConnectionNullReference(e);
                    await TryCloseConnectionAsync(force: true).ConfigureAwait(false);
                }
                catch (IOException e)
                {
        ...
        private async Task ProcessPDataTFAsync(PDataTF pdu)
        {
            try
            {
                for (var i = 0; i < pdu.PDVs.Count; i++)
                {
                    ...
                    Options.ReportBytesReceived?.Report((uint)pdv.Value.Length);
        ...
        private async Task PerformDimseAsync(DicomMessage dimse)
        {
            ...
            {
                Logger.InformationDicomMessageReceived(LogID, dimse.ToString(Options.LogDimseDatasets));
            }
        ...
        internal async Task SendNextMessageAsync()
        {
            ...
            while (true)
            {
                ...
                catch (Exception e)
                {
                    ...
                    if (msg is DicomRequest dicomRequest)
                    {
                        Logger.DebugRemovingRequestFromQueueBecauseOfError(dicomRequest.MessageID, e);
        ...
        private async Task DoSendMessageAsync(DicomMessage msg)
        {
            ...
            else
            {
                ...
                if (Logger.IsEnabled(LogLevel.Information))
                {
                    Logger.InformationDicomMessageSent(LogID, msg.ToString(Options.LogDimseDatasets));
                }
                ...
                try
                {
                    ...
                    using (var pCommandStream = new PDataTFStream(this, _memoryProvider, pc.ID, Association.MaximumPDULength, true, msg))
                    {

                        writer = new DicomWriter(
                        DicomTransferSyntax.ImplicitVRLittleEndian,
                        DicomWriteOptions.Default,
                            new StreamByteTarget(pCommandStream));

                    var commandWalker = new DicomDatasetWalker(msg.Command);
                    await commandWalker.WalkAsync(writer).ConfigureAwait(false);

                        await pCommandStream.FlushAsync(CancellationToken.None).ConfigureAwait(false);
                    }

                    if (msg.HasDataset)
                    {
                        // Use seperate DATA-TF for command and data (Some older DICOM receivers can't read the data when sent together in one DATA-TF)
                        using (var pDataStream = new PDataTFStream(this, _memoryProvider, pc.ID, Association.MaximumPDULength, false, msg))
                        {
        ...
        private async Task<bool> TryCloseConnectionAsync(Exception exception = null, bool force = false)
        {
            try
            {
                ...
                lock (_lock)
                {
                    ...
                    {
                        Logger.InformationTriedCloseConnectionWithNonemptyQueues(_pduQueue.Count, _msgQueue.Count, _pending.Count);
                        return false;
                    }
                }
            ...
            Logger.InformationConnectionClosed();
        ...
        protected Task SendAssociationRequestAsync(DicomAssociation association)
        {
            ...
            Logger.InformationAssociationRequenstSent(association.CalledAE, association);
        ...
        protected Task SendAssociationAcceptAsync(DicomAssociation association)
        {
            ...
            Logger.InformationAssociationAcceptSent(LogID, association);
        ...
        {
            ThrowIfAlreadyDisposed();
            Logger.InformationAssociationRejectSent(LogID, result, source, reason);
        ...
        internal class PDataTFStream : Stream
        {
            ...
            public PDataTFStream(DicomService service, IMemoryProvider memoryProvider, byte pcid, uint max, bool isCommand, DicomMessage dicomMessage)
            {
                ...
                // the configured maximum PDU-size depends if this is a command pdu or data pdu
                var pduMaxByOptions = isCommand ? _service.Options.MaxCommandBuffer : _service.Options.MaxDataBuffer;

                _max = max == 0
                    ? pduMaxByOptions // if the requested max pdu size is 0, then we use the configured default MaxSize
                    : Math.Min(max, pduMaxByOptions); // else we agree on the minimum of the requested MaxSize and the configured MaxSize
